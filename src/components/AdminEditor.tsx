...
type Tab = 'dashboard' | 'lots' | 'layout' | 'broker-layout' | 'brokers' | 'leads' | 'embed' | 'settings';
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'embed' && <EmbedGeneratorView />}
            {activeTab === 'settings' && (
              <SettingsView 
                config={config} 
                setConfig={setConfig} 
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        <div className="relative w-full md:w-96">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border border-brand-green/10 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-green/20 text-brand-green"
            value={search}
        ...
        {isDrawing && (
          <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-brand-green/10 shadow-sm">
            <input 
              ...
              className="w-24 px-4 py-2 bg-brand-offwhite rounded-xl text-sm focus:outline-none text-brand-green"
            ...
            <input 
              ...
              className="w-32 px-4 py-2 bg-brand-offwhite rounded-xl text-sm focus:outline-none text-brand-green"
              value={lotForm.area}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <input 
                  type="text" 
                  className="w-full px-4 py-3 bg-brand-offwhite rounded-2xl text-sm focus:outline-none text-brand-green"
...
function EmbedGeneratorView() {
  const [width, setWidth] = useState('100%');
  const [height, setHeight] = useState('600px');
  const [copied, setCopied] = useState(false);

  const embedCode = `<iframe 
  src="${window.location.origin}?view=client" 
  width="${width}" 
  height="${height}" 
  frameborder="0" 
...
></iframe>`;
  ...
  return (
    ...
    >
      <div className="bg-white p-8 rounded-3xl border border-brand-green/10 shadow-sm space-y-8">
        <div className="space-y-2">
          <h3 className="text-xl font-serif italic">Gerador de Mapa em HTML</h3>
          <p className="text-sm text-brand-green/60">Copie o código abaixo para incorporar o mapa interativo em seu site.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className="text-[10px] font-mono uppercase tracking-widest opacity-50">Largura (Width)</label>
            <input 
              type="text" 
              className="w-full px-4 py-3 bg-brand-offwhite rounded-2xl text-sm focus:outline-none text-brand-green"
              value={width}
          ...
          <div className="space-y-4">
            <label className="text-[10px] font-mono uppercase tracking-widest opacity-50">Altura (Height)</label>
            <input 
              type="text" 
              className="w-full px-4 py-3 bg-brand-offwhite rounded-2xl text-sm focus:outline-none text-brand-green"
              value={height}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <label className="text-[10px] font-mono uppercase tracking-widest opacity-50">Código HTML</label>
            <a 
              href={`${window.location.origin}?view=client`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[10px] font-mono uppercase tracking-widest text-brand-green hover:underline flex items-center gap-1"
            >
          ...
          <div className="relative">
            <pre className="w-full p-6 bg-brand-offwhite rounded-2xl text-xs font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed text-brand-green/80">
              {embedCode}
            </pre>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <button 
            ...
            className="flex-1 py-4 bg-brand-green text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-lg shadow-emerald-900/10"
          >
          ...
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 py-4 bg-white border border-brand-green/20 text-brand-green rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-green/5 transition-colors"
          >
        ...
        <div className="p-6 bg-amber-50 rounded-2xl border border-amber-100">
          <p className="text-xs text-amber-800 leading-relaxed">
            <strong>Dica:</strong> Você pode ajustar a largura e altura conforme necessário para que o mapa se adapte perfeitamente ao layout do seu site. O código acima já inclui bordas arredondadas e suporte a tela cheia.
          </p>
        </div>
      </div>
    </motion.div>
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: string) => {
    ...
    reader.onload = async (event) => {
      ...
      img.onload = async () => {
        ...
        // Max dimension for the map image to keep base64 under 1MB
        // 2000px is a good balance for quality vs size
        const MAX_DIMENSION = field === 'backgroundImage' ? 2400 : 800;
        ...
        if (ctx) {
          // Fill white background for JPEGs if needed, but we'll just draw
        ...
        if (compressedBase64.length > 1048487) { // Slightly less than 1MB to be safe
           alert('A imagem ainda está muito grande para o banco de dados. Tente uma imagem com menos detalhes ou menor resolução.');
           setIsUploading(false);
           return;
        }

        try {
          await handleUpdate({ [field]: compressedBase64, version: (config.version || 0) + 1 });
        } catch (err) {
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <input 
                    type="text" 
                    className="flex-1 px-3 py-2 bg-brand-offwhite rounded-lg text-xs font-mono text-brand-green"
                    value={config.primaryColor}
                    onChange={e => handleUpdate({ primaryColor: e.target.value })}
                  />
                </div>
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-6">
                ...
                <label className={cn(
                  "px-6 py-3 bg-brand-green text-white rounded-2xl text-sm font-bold cursor-pointer hover:scale-105 transition-transform flex items-center gap-2",
                  ...
                  {isUploading ? 'Processando...' : 'Upload Logo'}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <label className={cn(
                  "w-full flex items-center justify-center gap-2 px-6 py-4 bg-white border-2 border-brand-green/10 rounded-2xl text-sm font-bold cursor-pointer hover:bg-brand-green/5 transition-colors",
                  isUploading && "opacity-50 cursor-wait pointer-events-none"
                )}>
                  {isUploading ? (
                    <RefreshCw size={18} className="animate-spin" />
                  ...
                  {isUploading ? 'Processando Imagem...' : 'Alterar Imagem do Mapa'}
